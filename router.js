const express = require("express");
const bodyParser = require("body-parser");
const router = express.Router();
const sqlite3 = require('sqlite3').verbose();
const quizz = require('./db_function/quizz.js');
const score = require('./db_function/score.js');
const vote = require('./db_function/vote.js');
const user = require('./db_function/user.js');
console.log(quizz);
const db = new sqlite3.Database('./db/quizz');

module.exports = router;

/*
* Middleware
*/

router
    .use(bodyParser.json())
    .use(bodyParser.urlencoded({
    extended: true
}));

router.get("/", (req, res) => {
  res.json("Hello world!");
});

 Quizz 

// Quizz List //
router.get("/quizzes", (req, res) => {
  quizz.list(req, res);
});

// Particular Quizz Informations //
router.get('/quizz_info/:id', (req, res) => {
  quizz.info(req, res);
});

// Quizz Questions //
router.get('/quizz/:id', (req, res) => {
  quizz.questions(req, res);
});

// Quizz Answers //
router.get('/answers/:id', (req, res) => {
  quizz.answers(req, res);
});

// Create Quizz //
router.post('/quizz',(req, res) => {
  quizz.create(req, res);
});

// Create question //
router.post('/question',(req, res) => {
    quizz.create_question(req, res);
});

// Create answers //
router.post('/answers',(req, res) => {
    quizz.create_answers(req, res);
});

// delete questions //
router.delete('/question/:id',(req, res) => {
    quizz.drop_question(req, res);
});

// delete answers //
router.delete('/answers/:id',(req, res) => {
    quizz.drop_answers(req, res);
});

// Delete Quizz //
router.delete('/quizz/:id', (req, res) => {
    quizz.drop(req, res);
});

 Scores 

// All scores //
router.get('/scores', (req, res) => {
  score.all(req, res);
});

// Scores of a particular quizz //
router.get('/scores/:id', (req, res) => {
  score.quizz(req, res);
});

// Best scores of a particular quizz //
router.get('/highscores/:id', (req, res) => {
  score.quizzLeaderboard(req, res);
});

// Scores of a particular user //
router.get('/userscores/:id', (req, res) => {
  score.user(req, res);
});

// Score of a particular user for a giver quizz//
router.get('/score/:user_id/:quizz_id', (req, res) => {
  score.userQuizz(req, res);
});

// Add Score //
router.post('/score', (req, res) => {
  score.add(req, res);
});

 Vote 

// all votes //
router.get('/votes', (req, res) => {
  vote.all(req, res);
});

// Add vote //
router.post('/vote', (req, res) => {
  vote.add(req, res);
});

 Users 

// get user info //
router.get('/user/:id', (req, res) => {
  user.profile(req, res);
});

// get user info //
router.post('/user', (req, res) => {
  user.add(req, res);
});


 searchbar 

// Rrquette de recherche //
router.get('/search/:id', (req, res) => {
  let request01 = "SELECT * FROM users WHERE login LIKE '%"+ req.params.id +"%'"
  let request02 = "SELECT * FROM quizz WHERE name LIKE '%"+ req.params.id +"%'"
  db.all( request01, [], (err, rows) => {
    let result = [rows];
    db.all( request02, [], (err, rows) => {
      result.push(rows);
      res.json(result);
    });
  });
});

 Images 

// Upload Image quizz //
router.post('/upload/quizz', (req, res) => {
  req.files.file.mv(__dirname + '/resources/pictures/quizz' + req.files.file.name,
      (err) => {
        if (err)
          return res.status(500).send(err);
        res.json({file: req.files.file.name});
      }
    );
})

// Upload Image user //
router.post('/upload/users', (req, res) => {
  req.files.file.mv(__dirname + '/resources/pictures/users' + req.files.file.name,
      (err) => {
        if (err)
          return res.status(500).send(err);
        res.json({file: req.files.file.name});
      }
    );
})



/*
* 400 Errors handling
*/

router.use((req, res) => {
    res.status(400);
    res.json({
       error: "Bad request"
    });
});
